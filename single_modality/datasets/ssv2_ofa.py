...
class SSRawFrameClsDataset_OFA(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            index, self.num_segment, self.crop_size = index
            self.short_side_size = self.crop_size
    ...
    def load_frame(self, sample, num_frames, sample_rate_scale=1):
        ...
        # print(f'loading frames with total frame {self.num_segment}')
...
class SSVideoClsDataset_OFA(Dataset):
