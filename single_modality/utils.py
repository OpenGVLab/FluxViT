...
def save_model(
        ...
        model_ema=None, model_name=None, ceph_args={'use_ceph_checkpoint': False}, 
    ...
    if loss_scaler is not None:
        ...
        model.save_checkpoint(save_dir=local_save_dir, tag=tag_name, client_state=client_state, exclude_frozen_parameters=True)
        ...
        if loss_scaler is not None:
            ...
            if not flag:
                try:
                    ...
                    print(f"An exception occurred loading latest: {type(e).__name__}")
                    print('No latest model')
            if not flag:
                try:
                    load_specific_model(model, model_ema, args, output_dir, model_name='best')
