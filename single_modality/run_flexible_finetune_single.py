...
from engines.engine_for_flexible_tuning_single import train_one_epoch, validation_one_epoch, final_test, merge
...
def get_args():
    ...
    parser.add_argument('--eval_log_name', default='log-eval.txt', type=str,
                        help='string for splitting the evaluation log')
    # Model parameters
    ...
    parser.add_argument('--random_sample_a_merging', action='store_true', default=False)
    
    parser.add_argument('--eval_masking_ratio', type=float, default=0.5)
    
    parser.add_argument('--eval_orig_frame', type=int, default=0)
    ...
    parser.add_argument('--eval_true_frame', type=int, default=8)
    
    parser.add_argument('--eval_num_merging_to', nargs='+', type=int)
    parser.add_argument('--largest_num_merging_to', nargs='+', type=int)
    parser.add_argument('--middle_num_merging_to', nargs='+', type=int)
    parser.add_argument('--least_num_merging_to', nargs='+', type=int)
    
    parser.add_argument('--largest_num_input_token', type=int, default=4096)
    parser.add_argument('--middle_num_input_token', type=int, default=4096)
    parser.add_argument('--least_num_input_token', type=int, default=4096)
    parser.add_argument('--eval_max_token_num', type=int, default=3072)
    
    parser.add_argument('--random_mask_ratio', action='store_true', default=False)
    parser.add_argument('--uniform_merging_ratio', action='store_true', default=False)
    parser.add_argument('--long_schedule', action='store_true', default=False)
    
    parser.add_argument('--single_setting_training', action='store_true', default=False)
    
    parser.add_argument('--range_frame_start', type=int, default=6)
    parser.add_argument('--range_frame_end', type=int, default=26)
    parser.add_argument('--range_frame_stride', type=int, default=2)
    parser.add_argument('--range_res_start', type=int, default=112)
    parser.add_argument('--range_res_end', type=int, default=364)
    parser.add_argument('--range_res_stride', type=int, default=28)
    parser.add_argument('--limit_token_num', type=int, default=8192)
    parser.add_argument('--limit_min_token_num', type=int, default=2048)
    
    parser.add_argument('--model_res', type=int, default=336)
    ...
    parser.add_argument('--data_set', default='Kinetics', choices=[
        ...
        'SSV2', 'SSV2_ofa', 'UCF101', 'HMDB51', 'image_folder',
        'mitv1_sparse', 'Kinetics_sparse_ofa',
    ...
    parser.add_argument('--eval_input_size', default=224, type=int)
    parser.add_argument('--eval_short_side_size', default=224, type=int)

    parser.add_argument('--duplicate_aligner', action='store_true', default=False)
    
    parser.add_argument('--use_mlp_projector', action='store_true', default=False)

    parser.add_argument('--output_head', default=0, type=int)
    parser.add_argument('--first_single_then_triplet', action='store_true', default=False)
...
def main(args, ds_init):
    ...
    class CustomDistributedSampler(torch.utils.data.DistributedSampler):
        def __init__(self, dataset, args=None, num_replicas=None, rank=None, shuffle=True, seed=0, drop_last=False, res_list=None, frame_list=None, batch_size=None):
        ...
        def reset_input(self):
            args = self.args
            my_frame_list = range(args.range_frame_start, args.range_frame_end, args.range_frame_stride)
            my_res_s = range(args.range_res_start, args.range_res_end, args.range_res_stride)

            while True:
                self.num_segment = my_frame_list[torch.randint(0, len(my_frame_list), (1, )).item()]
                self.crop_size = my_res_s[torch.randint(0, (len(my_res_s)), (1, )).item()]
                n_tokens = self.num_segment * (self.crop_size // 14) ** 2 
                if n_tokens <= args.limit_token_num and n_tokens >= args.limit_min_token_num:
                    break
        ...
        def __iter__(self):
            ...
            for batch in batches:
                self.reset_input()
                for idx in batch:
                    yield (idx, self.num_segment, self.crop_size)

    sampler_train = CustomDistributedSampler(
        dataset_train, args=args, num_replicas=num_tasks, rank=global_rank, shuffle=True, batch_size=args.batch_size,
    )
    ...
    if args.eval:
        ...
        test_stats = final_test(data_loader_test, model, device, preds_file, args=args, ds=args.enable_deepspeed, bf16=args.bf16, test_time=0)
        torch.distributed.barrier()
        if global_rank == 0:
            print("Start merging results...")
            final_top1 ,final_top5 = merge(args.output_dir, num_tasks)
            print(f"Accuracy of the network on the {len(dataset_test)} test videos: Top-1: {final_top1:.2f}%, Top-5: {final_top5:.2f}%")
            log_stats = {'Eval Frame Num':  args.eval_true_frame,
                        'Eval Input Size': args.eval_input_size,
                        'Eval num': args.eval_max_token_num,
                        'Eval Merging To': args.eval_num_merging_to,
                        'Final top-1': final_top1,
                        'Final Top-5': final_top5}
            if args.output_dir and utils.is_main_process():
                with open(os.path.join(args.output_dir, args.eval_log_name), mode="a", encoding="utf-8") as f:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = validation_one_epoch(data_loader_val, model, device, args=args, ds=args.enable_deepspeed, bf16=args.bf16)
    ...
    test_stats = final_test(data_loader_test, model, device, preds_file, args=args, ds=args.enable_deepspeed, bf16=args.bf16)
    torch.distributed.barrier()
    if global_rank == 0:
        print("Start merging results...")
        final_top1 ,final_top5 = merge(args.output_dir, num_tasks)
